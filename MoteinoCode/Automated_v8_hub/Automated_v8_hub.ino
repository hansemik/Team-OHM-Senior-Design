...
#define SAMPLE_TIME     16 //(in seconds)
...
//        }
      ...
      for(nodes = 0; nodes < num_nodes; nodes++){
        ...
        rem = diff[nodes] % 10;
        if(rem == 0){
          samples_cut[num_nodes - nodes - 1] = (diff[nodes] / 10) + 1;
        }
        else if(rem >= 5){
          samples_cut[num_nodes - nodes - 1] = ((diff[nodes] + 10 - rem) / 10) + 1;
        }
        else if(rem < 5){
          samples_cut[num_nodes - nodes - 1] = ((diff[nodes] - rem) / 10) + 1;
        }
      }
  ...
  if(input == '4')
  {
      ...
      for(nodes = 0; nodes < num_nodes; nodes++)
      {
        for(i = 0; i < 8; i++)
        {
          ...
          if (currBlockMax > (3000 + (samples_cut[nodes] * 2)))
          {
            currBlockMax = (3000 + (samples_cut[nodes] * 2));
          }
