...
void loop() {
  ...
  if (Serial.available() > 0)
  {
    ...
    if (input == '1')
    {
      ...
      int nodes;
      int check = 0;
      uint32_t counter, sum, one, two;
      int diff, avg, maxi, mini;
      int delayTable[3][16] = {0};
...
//      radio.send(3, sending, strlen(sending));
//
//      counter = 0;
  ...
  if(input == '4')
  {
      ...
      for(nodes = 0; nodes < num_nodes; nodes++)
      {
        for(i = 0; i < 1; i++)
        {
          ...
          fonaSS.write(buffer);
          Serial.println(buffer);
          Serial.print("BLOCKS[nodes] = ");
          Serial.println(BLOCKS[nodes]);
          Serial.print("ChNStartPos[i] = ");
          Serial.println(ChNStartPos[i]);
          Serial.print("ChNCurrPos[i][nodes] = ");
          Serial.println(ChNCurrPos[i][nodes]);
